...
생일 케이크 엔딩이 나옵니다. **선풍기를 촛불 쪽으로 끌어다 놓으면** 촛불이
꺼지고 손편지가 한 글자씩 타이핑돼요. (잘 안 되면 촛불을 길게 눌러서도 끌 수 있어요.)
...
엔딩에서 화면 위의 **선풍기를 마우스/손가락으로 끌어** 촛불 쪽으로 가져가면
바람이 불어 촛불이 꺼집니다. 별도 권한(마이크 등)이 필요 없어 어디서든 동작해요.

- 잘 안 맞춰지면 **촛불을 1초 이상 길게 눌러서**도 끌 수 있어요.
- 감지 범위·유지 시간은 `js/main.js` 상단의 `BLOW_RANGE`, `BLOW_HOLD` 값으로 조절합니다.

> 참고: GitHub Pages는 `https://` 라 어떤 폰에서든 잘 열립니다.
